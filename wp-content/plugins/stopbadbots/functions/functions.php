...
if ($stop_bad_bots_autoupdate == 'yes')
    add_filter('auto_update_plugin', 'stopbadbots_auto_update', 10, 2);
...
if (version_compare(trim(STOPBADBOTSVERSION), trim($stopbadbots_version)) > 0) {
    ...
    if ($stopbadbots_bill_go_pro_hide == '') {
        ...
        if (!update_option('stopbadbots_bill_go_pro_hide', $today))
            add_option('stopbadbots_bill_go_pro_hide', $today);
    }
...
$stopbadbots_table = $wpdb->prefix . "sbb_fingerprint";
$query = "SELECT fingerprint from " . $stopbadbots_table . " 
...
if ($qrow < 1 and  !isset($_COOKIE['antihacker_cookie'])) {
    ...
    if (isset($_COOKIE['antihacker_cookie']) and empty($stopbadbots_fingerprint_filed))
        $stopbadbots_fingerprint_filed = esc_attr($_COOKIE['antihacker_cookie']);
    ...
    if (!empty($stopbadbots_fingerprint_filed) and $stop_bad_bots_engine_option != 'conservative') {
        ...
        if (gettype($afingerprint) == 'array') {
            ...
            if (count($afingerprint) > 1) {
               ...
               // die(var_dump($is_linux));

                //  die(var_dump(stopbadbots_find_ua_os($stopbadbots_userAgentOri)));
...
function stopbadbots_contextual_help()
{
    ...
    $myhelp .= '</a>&nbsp;';
    $myhelp .= __('for more details, Support and online guide.', 'stopbadbots');
    $myhelptable = '<br />';
    ...
    $screen->add_help_tab(array(
        ...
        'title' => __('Visits Log', 'stopbadbots'),
...
{
    global $bill_current_screen;
    $pointer_content = '<h3>Open Stop Bad Bots Plugin Here!</h3>';
    $pointer_content .= '<p>Just Click Over Stop Bad Bots, then Go To Settings=>StartUp Guide.';
    ?>
...
function stopbadbots_include_scripts()
{
    ...
    wp_register_script('sbb-scripts', STOPBADBOTSURL .
        'assets/js/stopbadbots_fingerprint.js', array('jquery'), null, true); //true = footer
...
function stopbadbots_gopro_callback9()
{
    ...
    echo '<script type="text/javascript">';
     'window.location = "' . esc_url($urlgopro) . '";';
    echo '</script>';
}
...
function stopbadbots_screen_options3()
{
    ...
    $args = array(
        'label' => __('Bad Referers per page', 'stopbadbots'),
        'default' => 10,
        'option' => 'stopbadbots_per_page'
    );
    add_screen_option('per_page', $args);
...
function sbb_alertme($stopbadbots_userAgentOri)
{
    ...
    $message[] = __('User Agent', 'stopbadbots') . "........: " . $stopbadbots_userAgentOri;
    ...
    $message[] = "";
    $message[] = __('Visit us to learn how to get Weekly Updates and more features:', 'stopbadbots');
...
function sbb_alertme3($stopbadbots_string)
{
    ...
    $message[] = __('Date', 'stopbadbots') . "..............: " . date("F j, Y, g:i a");
    $message[] = __('Robot IP Address', 'stopbadbots') . "..: " . $stopbadbotsip;
    $message[] = __('Malicious String Found:', 'stopbadbots') . " " . $stopbadbots_string;
    $message[] = "";
    $message[] = __('eMail sent by Stop Bad Bots Plugin.', 'stopbadbots');
    $message[] = __(
        'You can stop emails at the Notifications Settings Tab.',
...
function sbb_alertme4($stopbadbots_string)
{
    ...
    $message[] = "";
    $message[] = __('Date', 'stopbadbots') . "..............: " . date("F j, Y, g:i a");
    $message[] = __('Referer String Found:', 'stopbadbots') . " " . $stopbadbots_string;
    $message[] = "";
    $message[] = __('eMail sent by Stop Bad Bots Plugin.', 'stopbadbots');
    $message[] = __(
        'You can stop emails at the Notifications Settings Tab.',
...
function sbb_alertme8()
{
    ...
    $message[] = __('Fake User Agent:', 'stopbadbots') . " " . $stopbadbots_userAgentOri;
    $message[] = "";
    $message[] = __('Date', 'stopbadbots') . "..............: " . date("F j, Y, g:i a");
    $message[] = __('IP Found:', 'stopbadbots') . " " . $stopbadbotsip;
    $message[] = "";
...
{
    global $stopbadbotsserver, $sbb_admin_email, $stopbadbotsip;
    $subject = __("Blocked Bot by Fake Browser (User Agent)", "stopbadbots") . ' ' . $stopbadbotsserver;
    $message[] = __('Date', 'stopbadbots') . "..............: " . date("F j, Y, g:i a");
    $message[] = __('IP Found:', 'stopbadbots') . " " . $stopbadbotsip;
    $message[] = "";
    $message[] = __('eMail sent by Stop Bad Bots Plugin.', 'stopbadbots');
...
{
    global $stopbadbotsserver, $sbb_admin_email, $stopbadbotsip;
    $subject = __("Blocked Visitor by Country (China, Cuba, North Korea)", "stopbadbots") . ' ' . $stopbadbotsserver;
    $message[] = __('Date', 'stopbadbots') . "..............: " . date("F j, Y, g:i a");
    $message[] = __('IP Found:', 'stopbadbots') . " " . $stopbadbotsip;
    $message[] = "";
    $message[] = __('eMail sent by Stop Bad Bots Plugin.', 'stopbadbots');
...
function sbb_settings_init()
{
    ...
    add_settings_section('sbb_pluginPage_section', __(
        'Add new bad bot to the bad bots Table.',
        'stopbadbots'
    ), 'sbb_settings_section_callback', 'pluginPage');
    add_settings_field(
        'sbb_text_field_0',
...
function sbb_settings2_init()
{
    ...
    add_settings_section('sbb_pluginPage2_section', __(
        'Add new bad IP to the bad IPs Table.',
        'stopbadbots'
    ), 'sbb_settings_section2_callback', 'pluginPage2');
    add_settings_field(
        'sbb_text_field_2',
...
function sbb_settings3_init()
{
    ...
    add_settings_section('sbb_pluginPage3_section', __(
        'Add new bad Referer to the bad Referer Table.',
        'stopbadbots'
    ), 'sbb_settings_section3_callback', 'pluginPage3');
    add_settings_field(
        'sbb_text_field_3',
...
{ ?>
    <div class="notice notice-success is-dismissible">
        <p><?php _e('Referer included at table!', 'stopbadbots'); ?></p>
    </div>
<?php }
...
function sbb_update_db3()
{
    ...
    if (isset($options['sbb_input_ref'])) {
        ...
        $query = "INSERT INTO $table_name (botname,botstate,botdate,added) VALUES ('$ref', 'Enabled', now(), 'User')";
        $r = false;
        if (!empty($ref)) {
            $r = $wpdb->query($query);
            if ($r) {
...
function sbb_bill_ask_for_upgrade()
{
    ...
    if ($x == 0) {
        ...
        $banner_txt = __('Bad Bots can do all sorts of nasty stuff and waste server resources.', 'stopbadbots');
    } elseif ($x == 1) {
        $banner_image = STOPBADBOTSIMAGES . '/monitor-com-maca3.png';
        $bill_banner_bkg_color = 'orange';
        $banner_txt = __('Bad bots don’t play by the rules.', 'stopbadbots');
    } elseif ($x == 2) {
        $banner_image = STOPBADBOTSIMAGES . '/unlock-icon-red-small.png';
        $bill_banner_bkg_color = 'turquoise';
        $banner_txt = __('Bad Bots stresses, harm and slowly your Web servers.', 'stopbadbots');
    } elseif ($x == 3) {
        $banner_image = STOPBADBOTSIMAGES . '/5stars.png';
        $bill_banner_bkg_color = 'turquoise';
        $banner_txt = __('Show support with a 5-star rating.', 'stopbadbots');
    } elseif ($x == 4) {
        $banner_image = STOPBADBOTSIMAGES . '/special-offer.png';
        $bill_banner_bkg_color = 'turquoise';
        $banner_txt = __('BLACK FRIDAY 30% OFF! Use the coupon code: special-black_2019. Limited time!', 'stopbadbots');
    } else {
    ...
    $banner_tit = __('Stop Bad Bots Plugin. Its time to Get Pro Protection!', 'stopbadbots');
    /*
        ...
        <div class="sbb_bill_go_pro_message sbb_bill_banner_on_plugin_page sbb_bill_go_pro_banner">
            ...
            <div class="sbb_bill_go_pro_text">
                ...
                if ($x != '3')
                    ...
                    echo __('Help keep Stop Bad Bots plugin going strong!', 'stopbadbots');
                ?>
            </div>
            <div class="sbb_bill_go_pro_buttons">
                <div class="sbb_bill_go_pro_button_primary_container">
                    <?php if ($x != '3') {
                        ...
                        echo  __("Learn More", "stopbadbots");
                        echo '</a>';
                    } else {
                        echo '<a class="button button-primary" target="_blank" href="https://wordpress.org/support/plugin/stopbadbots/reviews/#new-post">';
                        echo  __("Go to WordPress", "stopbadbots");
                        echo '</a>';
                    } ?>
                </div>
                <div class="sbb_bill_go_pro_dismiss_container">
                    <a class="button button-secondary sbb_bill_go_pro_dismiss" target="_blank" href="https://stopbadbots.com/premium/"><?php _e(
                                                                                                                                            'Dismiss',
                                                                                                                                            'stopbadbots'
                                                                                                                                        ); ?></a>
                </div>
...
{
    $message = __('Your IP is blacklisted on Internet Public Databases. Please, use another way to contact us.', 'stopbadbots');
    return $message;
}
function validation_messages_fail2($message, $status)
{
    $message = __("Looks Like This Message doesn't come from our site. Please, use another way to contact us.", 'stopbadbots');
    return $message;
}
// ------------------------------------ 
...
function stopbadbots_find_ua_version($agent, $browser)
{
    if (empty($agent) or empty($browser))
        return "";
    $version = '';
    $pattern = '#(?<browser>' . join('|', ['Version', $browser, 'other']) . ')[/ ]+(?<version>[0-9.|a-zA-Z.]*)#';

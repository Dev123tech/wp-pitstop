...
$stopbadbots_request_url = esc_url($_SERVER['REQUEST_URI']);
...
if (is_admin() or is_super_admin()) {
    ...
    if (isset($_GET['page'])) {
        ...
        if ($page == 'stop_bad_bots_plugin' or $page == 'sbb_my-custom-submenu-page') {
...
$stop_bad_bots_autoupdate = sanitize_text_field(get_option('stop_bad_bots_autoupdate', ''));
...
$stopbadbots_bill_go_pro_hide = sanitize_text_field(get_option('stopbadbots_bill_go_pro_hide', ''));
...
$stop_bad_bots_engine_option = sanitize_text_field(get_option('stop_bad_bots_engine_option', 'normal'));
...
if (is_admin() or is_super_admin()) {
    require_once(STOPBADBOTSPATH . 'functions/health.php');
    function stopbadbots_add_admscripts()
    {
        ...
        if ($pos !== false) {
            ...
            wp_enqueue_script('sbb-botstrap', STOPBADBOTSURL .
        ...
        if ($pos !== false or $pos2 !== false or $pos3) {
            wp_enqueue_script('sbb-flot', STOPBADBOTSURL .
                'assets/js/jquery.flot.min.js', array('jquery'));
            wp_enqueue_script('sbb-flotpie', STOPBADBOTSURL .
                'assets/js/jquery.flot.pie.js', array('jquery'));
        }

        wp_enqueue_script('sbb-circle', STOPBADBOTSURL .
            'assets/js/radialIndicator.js', array('jquery'));
        wp_enqueue_script('sbb-easing', STOPBADBOTSURL .
            'assets/js/jquery.easing.min.js', array('jquery'));
        wp_enqueue_script('sbb-datatables10', STOPBADBOTSURL .
        ...
        wp_enqueue_script('botstrap40', STOPBADBOTSURL .
            'assets/js/dataTables.bootstrap4.min.js', array('jquery'));
        wp_enqueue_script('sbb-datatables20', STOPBADBOTSURL .
            'assets/js/dataTables.buttons.min.js', array('jquery'));
        wp_register_script('sbb-datatables_visitors_sbb', STOPBADBOTSURL .
            'assets/js/stopbadbots_table.js', array(), '1.0', true);
...
{
    wp_register_script('bots', STOPBADBOTSURL .
        'assets/js/stopbadbots.js', array('jquery'), null, true);
    wp_enqueue_script('bots');
}
...
if (!is_admin() and !is_super_admin()) {
    if ($stop_bad_bots_firewall != 'no' and $stopbadbots_checkversion != '') {
        ...
        if (isset($_SERVER['REQUEST_URI']) && !empty($_SERVER['REQUEST_URI'])) {
            $stopbadbots_request_uri_string = $_SERVER['REQUEST_URI'];
        }
        if (isset($_SERVER['QUERY_STRING']) && !empty($_SERVER['QUERY_STRING'])) {
            $stopbadbots_query_string_string = $_SERVER['QUERY_STRING'];
        }
...
function sbb_render_list_page3()
{
    ...
    require dirname(__FILE__) . '/includes/list-tables/page3.php';
}
...
function sbb_custom_dashboard_help()
{
    ...
    echo  '<h3><a href="' . esc_url($site) . '">For details, visit the plugin dashboard</a></h3>';
    echo '<br />';
    echo '<center><strong><big>Total Attacks Blocked Last 15 days</big></strong></center>';
    echo '<br />';
    ...
    echo '<br />';
    echo '<br />';
    echo '<center><strong><big>Total Attacks Blocked By Type</big></strong></center>';
    echo '<br />';
    require_once("dashboard/botsgraph_pie.php");
    echo '<br />';
    echo '<br />';
    echo '<br />';
    echo '<br />';
    echo '<hr>';
    echo '<br />';
    echo '<center><strong><big>Total Attacks Blocked By IP</big></strong></center>';
    echo '<br />';
    require_once "dashboard/topips.php";
    echo '<br />';
    echo '<br />';
...
if (($stopbadbots_install_anti_hacker != 'no' or $stopbadbots_install_recaptcha != 'no') and is_admin())
    require_once STOPBADBOTSPATH . '/includes/tgm/pinstaller.php';
...
function stop_bad_bots_install_required_extensions()
{
    ...
    echo '<div class="notice notice-warning is-dismissible">';
    echo '<br /><b>';
    echo __('Message from Stop Bad Bots Plugin', 'stopbadbots');
    echo ':</b><br />';
    echo __('To Install the extension:', 'stopbadbots');
    echo ' ' . esc_html($plugin_required);
    echo '<br />';
    echo ' <a class="button button-primary" href="plugins.php?page=tgmpa-install-plugins">';
    echo __('click here', "stopbadbots");
    echo '</a>';
    echo '<br /><br /></div>';
}
function stopbadbots_more_plugins()
{
    ...
    echo '<script>';
    echo 'window.location.replace("' . esc_url(STOPBADBOTSHOMEURL) . 'plugin-install.php?s=sminozzi&tab=search&type=author");';
    echo '</script>';
}
...
function stopbadbots_load_upsell()
{
    ...
    $stopbadbots_bill_go_pro_hide = trim(get_option('stopbadbots_bill_go_pro_hide'));
    // $stopbadbots_bill_go_pro_hide = '';
    // Debug ...
    $wtime = strtotime('-08 days');
    // update_option('stopbadbots_bill_go_pro_hide', $wtime);
    if (empty($stopbadbots_bill_go_pro_hide)) {
    ...
    if ($delta > (3600 * 24 * 6)) {
        ...
        if (!wp_script_is('bill-css-vendor-fix', $list)) {
            require_once(STOPBADBOTSPATH . 'includes/vendor/vendor.php');
            wp_enqueue_style('bill-css-vendor-fix', STOPBADBOTSURL . 'includes/vendor/vendor_fix.css');
        }
    }
    wp_register_script("bill-js-vendor-sbb", STOPBADBOTSURL . 'includes/vendor/vendor.js', array('jquery'), STOPBADBOTSVERSION, true);
    wp_enqueue_script('bill-js-vendor-sbb');
...
function stopbadbots_show_logo()
{
    ...
    // echo '<br>';
    echo '<img src="';
    echo STOPBADBOTSIMAGES . '/logo.png';
...
function stopbadbots_bill_go_pro_hide2()
{
    ...
    if (!update_option('stopbadbots_bill_go_pro_hide', $today))
        add_option('stopbadbotss_bill_go_pro_hide', $today);
    wp_die();
}
...
*/
/* =========================== */
// add_action('init','stopbadbots_add_cors_http_header');
/*
?page=tgmpa-install-plugins&
plugin=stopbadbots&tgmpa-activate=activate-plugin&tgmpa-nonce=1ceab601d0
https://stopbadbots.com/wp-admin/plugins.php?page=tgmpa-install-plugins
*/